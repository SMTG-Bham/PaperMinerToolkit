...
This guide gets PaperMinerToolkit running against a local vLLM server on Sol's Gaudi 2
...
recipe — override `PMT_RECIPE` and the query if you are after something else.

The important thing to understand first: **PaperMinerToolkit does no accelerator work
...
3.10; PaperMinerToolkit requires 3.11 or newer. They talk over `localhost` instead.

| Environment | Owner | Python | Contents |
| --- | --- | --- | --- |
| `gaudi-pytorch-vllm` | ASU Research Computing (shared) | 3.10 | vLLM + Habana PyTorch |
| `paperminertoolkit` | you | 3.14 | PaperMinerToolkit and its dependencies |
...
## 2. Create the PaperMinerToolkit environment

```bash
module load mamba/latest
mamba env create -f build_tools/environment.yml
source activate paperminertoolkit
```
...
`$PMT_ENV` already exists it is removed and recreated, so every run gives the same
environment regardless of what the last one left behind. Pass `PMT_FORCE=0` to
reuse an existing environment instead:

```bash
sbatch --export=ALL,PMT_FORCE=0 install.sbatch
```

It only ever touches `$PMT_ENV` — no other environment on the account — and
...
`pip` installs nothing but PaperMinerToolkit and its pure-Python dependencies into an
...
it instead; nothing in PaperMinerToolkit needs the host environment.
...
`nvidia-*` wheels and `triton` that are useless on a Gaudi node. PaperMinerToolkit only
...
missing `pmt_*` console script, or a failing test. It neither reads nor writes
...
also pre-caches the tokenizer for `PMT_MODEL`, so the Gaudi job does not need the
network for the chunk-sizing lookup. Pass `PMT_PREFETCH_WEIGHTS=1` to download
the model weights too, which keeps tens of GB of transfer out of your four-hour
accelerator allocation:

```bash
sbatch --export=ALL,PMT_PREFETCH_WEIGHTS=1 install.sbatch
```
...
This matters on both sides. vLLM downloads the weights, and PaperMinerToolkit loads
...
The interactive `pmt config elsevier-key`, `pmt config core-key`, `pmt config unpaywall-email`, and
`pmt config openalex-key` commands store these in `~/.config/.paperminertoolkitrc.json` if you
prefer.
...
- There is no "unlimited" flag in `pmt search`. Each backend loops until it has
...
return tens of thousands of papers, and `pmt download` then fetches all of them.
...
```bash
sbatch fetch_corpus.sbatch
sbatch --export=ALL,PMT_COUNT=500 fetch_corpus.sbatch
```
...
scheduler's spool directory — a relative `PMT_DB` is resolved against the
...
**The four-hour cap and an unbounded `PMT_COUNT` interact badly on a large
corpus**, and the two halves of the job behave differently if it is killed:

- `pmt search` upserts on DOI, so re-running it costs little and adds nothing
  twice.
- `pmt download` walks every row in the corpus and has **no skip for assets it
  already holds** — only abstracts short-circuit. A job that dies at the wall
  clock re-downloads everything on the next submit.

So either bound `PMT_COUNT` so the fetch finishes inside four hours, or switch the
...
`/v1/models` to answer, then runs `pmt scrape` and `pmt store` from your
...
```bash
sbatch -G 1 --export=ALL,PMT_TENSOR_PARALLEL_SIZE=1,PMT_MODEL=Qwen/Qwen2.5-7B-Instruct scrape_gaudi.sbatch
```

Override anything at submit time:

```bash
sbatch --export=ALL,PMT_RECIPE=polymer_db,PMT_COUNT=1 scrape_gaudi.sbatch
```

Watch it with `tail -f ps-gaudi-<jobid>.log`.

**Start with `PMT_COUNT=1`.** A single paper exercises prompt construction, the
chat call, and JSON parsing end to end, and fails in about a minute instead of
four hours.

### Scraping the whole corpus

`PMT_COUNT` is empty by default, which means every paper in `papers.db`. A corpus
...
- `pmt scrape` skips any stage already marked `succeeded`, so re-submitting the
...
**Do not use `PMT_COUNT` to pace a long run.** `--count` slices the *ordered
corpus*, not the unfinished part of it, so a second job with `PMT_COUNT=50` would
re-select the same first 50 papers, find them already succeeded, and do no new
work. Use it for testing (`PMT_COUNT=1`), never for resuming.

The one thing to watch is the CSV: `pmt scrape --output` appends across runs, and
`pmt store` matches against the existing columns, so keep one file per recipe and
...
`~/.config/.paperminertoolkitrc.json` and concurrent jobs would race over it.

### Checking the server by hand

If a run misbehaves, get an interactive Gaudi session, start the server, and
separate server problems from client problems before involving PaperMinerToolkit:
...
is what PaperMinerToolkit uses to load its tokenizer.

Then send the completion budget PaperMinerToolkit actually uses. This is the single
most useful check, because it is the one that catches the context-sizing mistake:

```bash
curl -s "http://127.0.0.1:$PORT/v1/chat/completions" \
  -H 'Content-Type: application/json' \
  -d "{\"model\":\"$PMT_MODEL\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}],\"max_tokens\":10000}"
```
...
1. **Text, 1 card** — `Qwen/Qwen2.5-7B-Instruct` at `PMT_TENSOR_PARALLEL_SIZE=1`.
   The cheapest thing to get queued, and the right rung for a first end-to-end
   test with `PMT_COUNT=1`.
2. **Text, 4 cards** — the default, `Qwen/Qwen3-30B-A3B-Instruct-2507`.
3. **Text, 8 cards** — the same model at `-G 8` and `PMT_TENSOR_PARALLEL_SIZE=8`,
   which Intel also validated. More KV headroom rather than a bigger model, so
   spend it on `PMT_MAX_MODEL_LEN` and `PMT_MAX_NUM_SEQS`, not on the weights.
4. **Vision** — see below.

### Choosing a model

`PMT_MODEL` has to satisfy three things at once, and the default is the model that
does.
...
**It must not be a thinking model.** `paperminertoolkit/extract.py` asks for a
...
PaperMinerToolkit defaults to `temperature=0, top_p=1` so extraction is reproducible.
Keep the deterministic defaults; if you see a run degenerate into repetition that
eats the completion budget, `PAPERMINERTOOLKIT_MODEL_TEMPERATURE` and
`PAPERMINERTOOLKIT_MODEL_TOP_P` override them per job.

### Context sizing — the one thing to get right

`PMT_MAX_MODEL_LEN` and `PMT_INPUT_TOKEN_LIMIT` are **not** the same number.

`paperminertoolkit/extract.py` requests a 10000-token completion on every call, and
...
| `PMT_MAX_MODEL_LEN` | `PMT_INPUT_TOKEN_LIMIT` | Use |
| --- | --- | --- |
...
`PMT_MAX_MODEL_LEN` and `PMT_INPUT_TOKEN_LIMIT` together, keeping the 11000-token
gap.
...
| Knob | Default | Why |
| --- | --- | --- |
| `PMT_BLOCK_SIZE` | `128` | the KV block size the Gaudi backend is tuned for |
| `PMT_GPU_MEMORY_UTILIZATION` | `0.80` | leaves device memory free for HPU graph capture |
| `PMT_MAX_NUM_SEQS` | `16` | caps concurrent sequences, and so KV cache growth |

If warmup dies on an allocation failure, lower `PMT_GPU_MEMORY_UTILIZATION`
before touching anything else:

```bash
sbatch --export=ALL,PMT_GPU_MEMORY_UTILIZATION=0.70 scrape_gaudi.sbatch
```

### Adding vision

PaperMinerToolkit keeps separate `text` and `vision` profiles, so the text model does
...
```bash
sbatch -G 1 --export=ALL,PMT_TENSOR_PARALLEL_SIZE=1,PMT_MODEL=Qwen/Qwen2.5-VL-7B-Instruct,ENDPOINT_FILE=$PWD/vllm_vision_endpoint.txt \
       serve_gaudi.sbatch
...
```bash
export PAPERMINERTOOLKIT_VISION_MODEL_PROVIDER=local
export PAPERMINERTOOLKIT_VISION_MODEL_NAME=Qwen/Qwen2.5-VL-7B-Instruct
export PAPERMINERTOOLKIT_VISION_MODEL_BASE_URL="http://$(cat vllm_vision_endpoint.txt)/v1"
export PAPERMINERTOOLKIT_VISION_MODEL_CAPABILITIES=text,vision
```

Then run with `PMT_MODE=text-images`. If you raise `pmt scrape --image-batch-size`
...
Raise `PMT_STARTUP_TIMEOUT`, or uncomment `VLLM_SKIP_WARMUP=true` while iterating —
...
`PMT_MAX_MODEL_LEN`, `PMT_GPU_MEMORY_UTILIZATION`, or `PMT_MAX_NUM_SEQS` — see
Gaudi launch flags above.

**The job dies immediately with a card-count error.** `PMT_TENSOR_PARALLEL_SIZE`
...
submit time: `sbatch -G 1 --export=ALL,PMT_TENSOR_PARALLEL_SIZE=1 …`. Left
...
**Chunking looks wrong and nothing is logged.** PaperMinerToolkit falls back to a
`len/3` character estimate whenever the tokenizer fails to load, and it does so
silently. Confirm `PMT_MODEL` is the exact Hugging Face repo id and that `HF_HOME`
is set in the job.

**The scrape produces no rows, but the job "succeeded".** This is the failure
mode to internalise. `pmt scrape` catches exceptions per paper, records them on
...
**`pmt download` fails from a compute node.** Sol sits behind static NAT so
...
**The job hangs after scraping.** `pmt store` prompts for confirmation without
...
If you change `PMT_MODEL`, check it against Intel's
[validated models list](https://docs.vllm.ai/projects/gaudi/en/latest/getting_started/validated_models.html)
for a Gaudi **2** entry and confirm the tensor parallel size it was validated at,
then update the `-G` header, `PMT_TENSOR_PARALLEL_SIZE`, and the tokenizer default
