...
# Every paper the sources will give us. pmt search has no "unlimited" flag: each
...
COUNT="${3:-1000000}"
PMT_ENV="${PMT_ENV:-paperminertoolkit}"
...
pmt search "$QUERY" "$DB" --source all --count "$COUNT"

echo "=== Downloading ==="
pmt download "$DB" --format both --source all
...
echo "  sbatch --export=ALL,PMT_DB=$DB scrape_gaudi.sbatch"
echo
echo "Start with one paper to check the pipeline end to end:"
echo "  sbatch --export=ALL,PMT_DB=$DB,PMT_COUNT=1 scrape_gaudi.sbatch"
