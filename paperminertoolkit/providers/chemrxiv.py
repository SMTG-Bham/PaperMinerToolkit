"""Request helpers for the chemRxiv API used by PaperMinerToolkit.

This module centralizes chemRxiv HTTP details and the mapping from chemRxiv API
records onto PaperMinerToolkit's paper schema, so search, download, and enrichment
...
why :mod:`paperminertoolkit.workflows.search` pages chemRxiv the way it pages arXiv instead of
...
:data:`paperminertoolkit.workflows.download.TEXT_SOURCES`.

**The version suffix is part of a chemRxiv DOI and is never stripped.** This is
the one place where copying :mod:`paperminertoolkit.providers.biorxiv` would be wrong. A
...
def request_headers() -> dict[str, str]:
    ...
    The headers name PaperMinerToolkit honestly. chemrxiv.org runs a bot challenge
...
def request(
    ...
    request was rejected has no way to tell that from a bad query. PaperMinerToolkit
    ...
    def challenge(response: provider.ResponseLike) -> str:
        ...
        return (f'chemRxiv refused the request with 403 from {url}. chemrxiv.org is '
                f'behind a bot challenge that PaperMinerToolkit does not try to bypass; '
...
def normalize_chemrxiv_doi(value: object) -> str:
    ...
    the version away, as :func:`paperminertoolkit.providers.biorxiv.normalize_biorxiv_doi`
...
def record_to_paper(record: Mapping[str, Any]) -> _ChemrxivRecord:
    """Map one chemRxiv API record onto PaperMinerToolkit's paper schema.
...
def latest_versions(entries: Sequence[Mapping[str, Any]]) -> list[_ChemrxivRecord]:
    ...
    recent revision. That matches how :mod:`paperminertoolkit.providers.arxiv` and
    :mod:`paperminertoolkit.providers.biorxiv` date a revised preprint, which is what keeps a
