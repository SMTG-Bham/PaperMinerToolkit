"""Request helpers for the CORE API used by PaperMinerToolkit.
...
none is better off not asking. Store one with ``pmt config core-key`` or in
...
def request_headers(api_key: str | None = None) -> dict[str, str]:
    ...
    -------
    dict[str, str]
        Headers containing the PaperMinerToolkit user agent and, when available, a
        bearer token.
    """
...
def work_to_paper(work: Mapping[str, Any]) -> _CoreRecord:
    """Map one CORE work onto PaperMinerToolkit's paper schema.
