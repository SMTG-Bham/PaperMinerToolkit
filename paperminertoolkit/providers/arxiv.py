"""Request helpers for the arXiv API used by PaperMinerToolkit.

This module centralizes arXiv HTTP details and the mapping from arXiv Atom
entries onto PaperMinerToolkit's paper schema, so search, download, and enrichment
...
def entry_to_paper(entry: ET.Element) -> _ArxivRecord:
    """Map one arXiv entry onto PaperMinerToolkit's paper schema.
...
def query_expression(query: str, default_field: str = 'all') -> str:
    ...
    ``pmt search`` hands every provider the same bare phrase, but arXiv's
