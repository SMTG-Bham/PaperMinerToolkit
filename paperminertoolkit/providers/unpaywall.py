"""Request helpers for the Unpaywall API used by PaperMinerToolkit.
...
with ``pmt config unpaywall-email`` or in ``UNPAYWALL_EMAIL``.
...
def get_work(doi: object,
    ...
    if not address:
        raise ValueError('Unpaywall email is not configured. Run pmt config unpaywall-email first.')
