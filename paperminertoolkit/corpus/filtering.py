...
from paperminertoolkit.corpus.database import connect, get_asset, paper_rows, utc_now
from paperminertoolkit.corpus.documents import read_pdf_bytes, trim_reference_section
...
def topic_filter_staleness(conn: sqlite3.Connection) -> dict[str, bool]:
    ...
    if not filters:
        return {}
    from paperminertoolkit.workflows.topics import topic_corpus_fingerprint
...
def current_filter_statuses(conn: sqlite3.Connection) -> dict[str, _FilterStatus]:
    ...
    if stale:
        raise ValueError(
            'Active topic filters use stale scores; run pmt topics store again for: '
            + ', '.join(stale)
        )
    return {
