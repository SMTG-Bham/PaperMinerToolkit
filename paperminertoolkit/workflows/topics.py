...
def train_topic_model(db_path: str | PathLike[str],
    ...
    if streaming:
        ...
        with tempfile.TemporaryDirectory(
                prefix='paperminertoolkit-topics-', dir=cache_dir) as temporary_dir:
...
def store_topic_model_scores(
    ...
    with tempfile.TemporaryDirectory(prefix='paperminertoolkit-topic-store-') as temporary_dir:
        ...
        with connect(db_path) as conn, sqlite3.connect(staging_path) as staging:
            ...
            try:
                ...
                raise
    try:
        from paperminertoolkit.corpus.filtering import refresh_topic_filters
...
def plot_topic_trends(
    ...
    try:
        ...
        raise RuntimeError(
            'Plotting topic trends requires matplotlib. Install PaperMinerToolkit dependencies '
            'and rerun pmt topics trends with --plot.'
        ) from error
    ...
    try:
        if streaming:
            ...
            temporary = tempfile.TemporaryDirectory(
                prefix='paperminertoolkit-topics-compare-', dir=cache_dir
            )
            prepared = _prepare_streaming_corpus(
