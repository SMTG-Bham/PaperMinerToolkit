...
module holds the implementation once; :mod:`paperscraper.medrxiv` and
:mod:`paperscraper.biorxiv` supply those four differences as a
...
:func:`matches` exist: :mod:`paperscraper.search` walks intervals and applies
...
def record_to_paper(server: RxivServer, record: Mapping[str, Any]) -> RxivRecord:
    """Map one API record onto PaperScraper's paper schema.
...
def full_text(server: RxivServer,
    ...
    serves, so :mod:`paperscraper.pubmed`'s flattener is reused rather than
