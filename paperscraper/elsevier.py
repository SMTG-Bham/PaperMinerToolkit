...
records onto PaperScraper's paper schema.
...
def configured_api_key(settings: Mapping[str, str] | None = None) -> str:
    ...
    settings : Mapping[str, str] or None, optional
        Loaded PaperScraper settings. Read from disk when omitted.
...
def record_to_paper(record: Mapping[str, Any]) -> dict[str, Any]:
    """Map one Elsevier search record onto PaperScraper's paper schema.
...
def check_api_key(api_key: str, session: provider.HTTPClient | None = None) -> bool:
    ...
    This lives here rather than in :mod:`paperscraper.settings` so that settings
