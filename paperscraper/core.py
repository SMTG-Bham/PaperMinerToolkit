"""Request helpers for the CORE API used by PaperScraper.
...
def request_headers(api_key: str | None = None) -> dict[str, str]:
    ...
    -------
    dict[str, str]
        Headers containing the PaperScraper user agent and, when available, a
        bearer token.
    """
...
def work_to_paper(work: Mapping[str, Any]) -> _CoreRecord:
    """Map one CORE work onto PaperScraper's paper schema.
