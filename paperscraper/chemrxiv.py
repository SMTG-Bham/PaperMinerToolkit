"""Request helpers for the chemRxiv API used by PaperScraper.
...
why :mod:`paperscraper.search` pages chemRxiv the way it pages arXiv instead of
...
:data:`paperscraper.download.TEXT_SOURCES`.

**The version suffix is part of a chemRxiv DOI and is never stripped.** This is
the one place where copying :mod:`paperscraper.biorxiv` would be wrong. A
...
def request_headers() -> dict[str, str]:
    ...
    The headers name PaperScraper honestly. chemrxiv.org runs a bot challenge
...
def request(
    ...
    request was rejected has no way to tell that from a bad query. PaperScraper
    ...
    try:
        ...
        if (response.text or '').lstrip()[:1] == '<':
            raise RuntimeError(
                ...
                f'chemrxiv.org is behind a bot challenge that PaperScraper does not try '
...
def normalize_chemrxiv_doi(value: object) -> str:
    ...
    the version away, as :func:`paperscraper.biorxiv.normalize_biorxiv_doi`
...
def record_to_paper(record: Mapping[str, Any]) -> _ChemrxivRecord:
    """Map one chemRxiv API record onto PaperScraper's paper schema.
...
def latest_versions(entries: Sequence[Mapping[str, Any]]) -> list[_ChemrxivRecord]:
    ...
    recent revision. That matches how :mod:`paperscraper.arxiv` and
    :mod:`paperscraper.biorxiv` date a revised preprint, which is what keeps a
