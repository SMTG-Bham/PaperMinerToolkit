...
module holds the implementation once; :mod:`paperminer.medrxiv` and
:mod:`paperminer.biorxiv` supply those four differences as a
...
:func:`matches` exist: :mod:`paperminer.search` walks intervals and applies
...
from paperminer import provider, pubmed
...
def latest_versions(server: RxivServer,
    ...
    recent revision. That matches how :mod:`paperminer.arxiv` dates a
...
def full_text(server: RxivServer,
    ...
    serves, so :mod:`paperminer.pubmed`'s flattener is reused rather than
