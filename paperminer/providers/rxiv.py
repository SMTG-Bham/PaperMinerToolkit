...
module holds the implementation once; :mod:`paperminer.providers.medrxiv` and
:mod:`paperminer.providers.biorxiv` supply those four differences as a
...
:func:`matches` exist: :mod:`paperminer.workflows.search` walks intervals and applies
...
from paperminer.providers import base as provider, pubmed
...
def record_to_paper(server: RxivServer, record: Mapping[str, Any]) -> RxivRecord:
    """Map one API record onto PaperMiner's paper schema.
...
def latest_versions(server: RxivServer,
    ...
    recent revision. That matches how :mod:`paperminer.providers.arxiv` dates a
...
def full_text(server: RxivServer,
    ...
    serves, so :mod:`paperminer.providers.pubmed`'s flattener is reused rather than
