...
why :mod:`paperminer.workflows.search` pages chemRxiv the way it pages arXiv instead of
...
:data:`paperminer.workflows.download.TEXT_SOURCES`.

**The version suffix is part of a chemRxiv DOI and is never stripped.** This is
the one place where copying :mod:`paperminer.providers.biorxiv` would be wrong. A
...
def normalize_chemrxiv_doi(value: object) -> str:
    ...
    the version away, as :func:`paperminer.providers.biorxiv.normalize_biorxiv_doi`
...
def latest_versions(entries: Sequence[Mapping[str, Any]]) -> list[_ChemrxivRecord]:
    ...
    recent revision. That matches how :mod:`paperminer.providers.arxiv` and
    :mod:`paperminer.providers.biorxiv` date a revised preprint, which is what keeps a
