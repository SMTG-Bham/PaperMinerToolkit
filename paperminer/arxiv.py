"""Request helpers for the arXiv API used by PaperMiner.

This module centralizes arXiv HTTP details and the mapping from arXiv Atom
entries onto PaperMiner's paper schema, so search, download, and enrichment
...
def entry_to_paper(entry: ET.Element) -> _ArxivRecord:
    """Map one arXiv entry onto PaperMiner's paper schema.
