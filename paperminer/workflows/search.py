...
from paperminer.providers import (arxiv, biorxiv, chemrxiv, core, elsevier, medrxiv,
                          openalex, pubmed)
from paperminer.workflows.enrichment import enrich_papers
from paperminer.corpus.database import PAPER_FIELDS, add_asset, connect, find_paper, normalize_paper, upsert_paper, upsert_papers
...
def arxiv_search(query: str, count: int = 200) -> pd.DataFrame:
    ...
    :func:`paperminer.providers.arxiv.query_expression`; a native arXiv expression is
...
def _rxiv_search(provider: ModuleType,
    ...
    provider : types.ModuleType
        :mod:`paperminer.providers.medrxiv` or :mod:`paperminer.providers.biorxiv`.
    label : str
    ...
    if not total:
        return _rxiv_rows([])
    print(f'{label} has no search endpoint, so PaperMiner is reading the {total} postings '
...
def medrxiv_search(query: str, count: int = 200, today: str = '') -> pd.DataFrame:
    ...
    One search reads at most :data:`paperminer.providers.medrxiv.MAX_SCAN_RECORDS`
...
def biorxiv_search(query: str, count: int = 200, today: str = '') -> pd.DataFrame:
    ...
    :data:`paperminer.providers.biorxiv.MAX_SCAN_RECORDS` limit is correspondingly
...
def chemrxiv_search(query: str, count: int = 200) -> pd.DataFrame:
    ...
    records are grouped by :func:`paperminer.providers.chemrxiv.chemrxiv_stem` and
    ...
    :data:`paperminer.providers.chemrxiv.MAX_SEARCH_RESULTS` matches for any query, so
