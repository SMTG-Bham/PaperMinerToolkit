"""Request helpers for the bioRxiv API used by PaperMiner.

This module gives bioRxiv its own names and constants over the shared
bioRxiv-family client in :mod:`paperminer._rxiv`, which documents the paging,
...
def endpoint(category: str = '') -> tuple[str, int]:
    ...
    See :func:`paperminer._rxiv.endpoint` for why the two hosts differ and
...
def record_to_paper(record: Mapping[str, Any]) -> _BiorxivRecord:
    """Map one bioRxiv API record onto PaperMiner's paper schema.
