...
from paperminer import (arxiv, biorxiv, chemrxiv, core, elsevier, medrxiv,
                          openalex, pubmed)
from paperminer.enrichment import enrich_papers
from paperminer.corpus import PAPER_FIELDS, add_asset, connect, find_paper, normalize_paper, upsert_paper, upsert_papers
...
def arxiv_search(query: str, count: int = 200) -> pd.DataFrame:
    ...
    :func:`paperminer.arxiv.query_expression`; a native arXiv expression is
...
def _rxiv_search(provider: ModuleType,
    ...
    provider : types.ModuleType
        :mod:`paperminer.medrxiv` or :mod:`paperminer.biorxiv`.
    label : str
...
def medrxiv_search(query: str, count: int = 200, today: str = '') -> pd.DataFrame:
    ...
    One search reads at most :data:`paperminer.medrxiv.MAX_SCAN_RECORDS`
...
def biorxiv_search(query: str, count: int = 200, today: str = '') -> pd.DataFrame:
    ...
    :data:`paperminer.biorxiv.MAX_SCAN_RECORDS` limit is correspondingly
...
def chemrxiv_search(query: str, count: int = 200) -> pd.DataFrame:
    ...
    records are grouped by :func:`paperminer.chemrxiv.chemrxiv_stem` and
    ...
    :data:`paperminer.chemrxiv.MAX_SEARCH_RESULTS` matches for any query, so
