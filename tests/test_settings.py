...
def test_update_model_settings_prompts_and_saves_profile(isolated_settings_file, monkeypatch):
    """
    ...
    Asserts:
        - The interactive responses are persisted in the selected profile.
    """
    responses = iter([
        'vision',
        'local',
...
def test_update_model_settings_rejects_missing_model_name(isolated_settings_file, monkeypatch):
    """
    Test validation of the interactive model profile update flow.

    This function performs the following steps:
    1. Supplies prompt responses with an empty model name.
    2. Calls `update_model_settings`.
    3. Captures the expected validation error.

    Asserts:
        - A missing model name raises `ValueError`.
    """
    responses = iter([
        'text',
        'openai',
