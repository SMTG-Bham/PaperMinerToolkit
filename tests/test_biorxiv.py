...
class FakeResponse:
    """Prepared bioRxiv response with a configurable status code and body."""
...
class FakeSession:
    """Return prepared bioRxiv responses and record request arguments."""
...
def reset_pacer(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(biorxiv, '_last_request_at', 0.0, raising=False)
...
def test_request_paces_consecutive_calls_with_the_courtesy_delay(
    ...
    sleeps: list[float] = []
    clock = {'now': 100.0}
    monkeypatch.setattr(biorxiv.time, 'sleep', lambda seconds: sleeps.append(seconds))
    monkeypatch.setattr(biorxiv.time, 'monotonic', lambda: clock['now'])
...
def test_interval_page_sends_the_category_to_the_host_that_applies_it() -> None:
    ...
    assert session.calls[0]['headers']['User-Agent'] == biorxiv.USER_AGENT
