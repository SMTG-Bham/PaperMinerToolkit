...
```bash
ps_topics_train papers.db topic_model \
  --topics 8 \
  --field abstract \
  --stopwords-file domain_stopwords.txt \
  --batch-size 1000 \
...
ps_topics_show topic_model --representatives 5
...
After storing a model, use `ps_filter_topic` alone or compose it with regex filters. See {doc}`filtering` for the definition schema, stale-score protection, and hybrid stack semantics.
