...
Recipes define the structured records that PaperMinerToolkit asks a language or vision model to extract. They are not restricted to materials: a record can represent a material, experiment, reaction, device, organism, intervention, measurement series, or another unit that can be described by a paper.
...
Use the same recipe for `pmt scrape` and `pmt store`. Mixing recipes can produce incompatible columns, aliases, and units.
...
```bash
pmt scrape papers.db ./cycling_recipe.json --mode text
pmt store papers.db temp_scraped_materials.csv cycling_results.csv ./cycling_recipe.json --assume-yes
```
...
`plural`
: Its explicit plural form. PaperMinerToolkit does not guess plurals because scientific terms and multi-word phrases are not reliably pluralised automatically.
...
`aliases`
: Optional alternative headings accepted by `pmt store`. Aliases do not change the extraction prompt. Keep aliases unique across fields to avoid ambiguous storage matches.

`unit`
: Optional target unit used by `pmt store`. It does not silently rewrite the extraction prompt. During storage, supported non-missing values can be converted to the configured unit and the final column receives the unit suffix.
...
PaperMinerToolkit constructs a system prompt in a fixed order:
...
Recipe values are inserted only into fixed prompt sections. PaperMinerToolkit does not run arbitrary string formatting over `additional prompts`, so braces in JSON examples or scientific notation are preserved literally.
...
When `pmt store` converts the `Capacity` field to its recipe unit of `mAh g^-1`, it sends this system prompt:
