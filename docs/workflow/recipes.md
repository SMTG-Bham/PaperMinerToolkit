...
Use the same recipe for `pm scrape` and `pm store`. Mixing recipes can produce incompatible columns, aliases, and units.
...
```bash
pm scrape papers.db ./cycling_recipe.json --mode text
pm store papers.db temp_scraped_materials.csv cycling_results.csv ./cycling_recipe.json --assume-yes
```
...
`aliases`
: Optional alternative headings accepted by `pm store`. Aliases do not change the extraction prompt. Keep aliases unique across fields to avoid ambiguous storage matches.

`unit`
: Optional target unit used by `pm store`. It does not silently rewrite the extraction prompt. During storage, supported non-missing values can be converted to the configured unit and the final column receives the unit suffix.
...
When `pm store` converts the `Capacity` field to its recipe unit of `mAh g^-1`, it sends this system prompt:
...
```python
from paperminer.extraction.extract import build_image_extraction_prompt, build_text_extraction_prompt
from paperminer.extraction.recipes import load_recipe
